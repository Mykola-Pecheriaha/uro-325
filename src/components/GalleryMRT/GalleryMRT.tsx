...
import styles from './GalleryMRT.module.css';
...
interface GalleryMRTProps {
  images: ImageType[];
  galleryId: string;
  initialZoom?: number;
}

const GalleryMRT: FC<GalleryMRTProps> = ({
  images,
  galleryId,
  initialZoom = 1,
}) => {
      ...
      >
        <div className={styles.imageContainer}>
          <Image
            ...
            alt={`Зображення МРТ ${currentImageIndex + 1}`}
            width={scaledWidth}
            height={scaledHeight}
            layout="responsive"
            objectFit="cover"
          ...
          <div className={styles.indicators}>
            {images.map((_, index) => (
              <button
                ...
                aria-label={`Перейти до зображення МРТ ${index + 1}`}
